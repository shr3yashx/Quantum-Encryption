...
def transmit_qubits(qubits, eavesdrop_probability=0.0):
    ...
    for bit, base in qubits:
        if random.random() < eavesdrop_probability:  # Simulate eavesdropper interference
...
def measure_qubits(qubits, bob_bases):
    ...
    for (bit, alice_base), bob_base in zip(qubits, bob_bases):
        if alice_base == bob_base:  # Correct basis
            measured_bits.append(bit)
        else:  # Incorrect basis, random measurement
...
# Encryption and Decryption Functions (XOR-based)
